...
export function ProductCard({ 
  ...
  return (
    // Adicionei 'group' para facilitar efeitos de hover no futuro
    <Card className="w-3xs flex justify-center group relative overflow-hidden">
      
      {/* 2. HEADER RELATIVO PARA O BOTÃO FLUTUAR */}
      <CardHeader className="py-0 h-[210px] flex items-center justify-center relative bg-zinc-50"> 
        
        {/* --- AQUI ESTÁ O BOTÃO DE FAVORITOS --- */}
        <div className="absolute top-2 right-2 z-10">
            <WishlistButton product={product} className="bg-white/80 backdrop-blur-sm shadow-sm" />
        </div>
        {/* -------------------------------------- */}

        <img className="object-cover max-h-full" src={imagePath} alt={product.name} />
      </CardHeader>

      <CardContent className="pt-4">
        <h4 className="font-semibold text-lg line-clamp-2 min-h-[3.5rem] mb-2">{product.name}</h4>
